...
do
  ...
  then 
    ...
    then
            cd ~/komodo/src
            RESULT="$(./komodo-cli -rpcclienttimeout=15 listunspent | grep .0005 | wc -l)"
            RESULT1="$(./komodo-cli -rpcclienttimeout=15  listunspent|grep amount|awk '{print $2}'|sed s/.$//|awk '$1 < 0.0005'|wc -l)"
            RESULT2="$(./komodo-cli -rpcclienttimeout=15 getbalance)"
    fi
    if [ "$count" = "2" ]
    then
            RESULT="$(bitcoin-cli -rpcclienttimeout=15 listunspent | grep .0005 | wc -l)"
            RESULT1="$(bitcoin-cli -rpcclienttimeout=15  listunspent|grep amount|awk '{print $2}'|sed s/.$//|awk '$1 < 0.0005'|wc -l)"
            RESULT2="$(bitcoin-cli -rpcclienttimeout=15 getbalance)"

    fi
    if [ "$count" = "3" ]
    then
            RESULT="$(chips-cli -rpcclienttimeout=15 listunspent | grep .0005 | wc -l)"
            RESULT1="$(chips-cli -rpcclienttimeout=15  listunspent|grep amount|awk '{print $2}'|sed s/.$//|awk '$1 < 0.0005'|wc -l)"
    ...
    then
            cd ~/komodo/src
            RESULT="$(./komodo-cli -rpcclienttimeout=15 -ac_name=${processlist[count]} listunspent | grep .0005 | wc -l)"
            RESULT1="$(./komodo-cli -ac_name=${processlist[count]} -rpcclienttimeout=15  listunspent|grep amount|awk '{print $2}'|sed s/.$//|awk '$1 < 0.0005'|wc -l)"
